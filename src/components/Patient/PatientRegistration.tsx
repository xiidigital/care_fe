...
export const PatientRegistration = ({ patientId }: { patientId?: string }) => {
  ...
  useEffect(() => {
    ...
    if (!data) {
      ...
      form.setValue("geo_organization", facility.geo_organization.id);
      form.setValue("_selected_levels", [facility.geo_organization]);
      return;
    }
    ...
    form.reset({
      ...
      region_id: data.location?.region?.id,
      subregion_id: data.location?.subregion?.id,
  ...
  }, [patientQuery.data, facility, ext.defaults]);
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (!patientId) {
      createPatient({
        ...
        registration_facility: facilityId,
      });
    } else {
      updatePatient({
        ...basePayload,
...
const PatientBasicsContent = ({
  ...
  const facilityCountryCode = facility?.geo_organization.geography?.country
    ?.country_code as CountryCode | undefined;
            ...
            <FormItem>
              ...
              <FormControl>
                <PhoneInput
                  ...
                  defaultCountry={facilityCountryCode}
                  onChange={(value) => {
  ...
  return (
    <>
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            <FormLabel>{postalCode.label}</FormLabel>
            <FormControl>
              <Input
                {...field}
                value={field.value ?? ""}
      ...
      <GeographicLocationFields
        ...
        onChange={(location) => {
          form.setValue("region_id", location.region_id, { shouldDirty: true });
          form.setValue("subregion_id", location.subregion_id, {
            shouldDirty: true,
          });
          form.setValue("city_id", location.city_id, { shouldDirty: true });
        }}
      />
...
const getFormSchema = (
  ...
  return z
    .object({
      ...
      geo_organization: z.uuid({ message: t("geo_organization_is_required") }),
